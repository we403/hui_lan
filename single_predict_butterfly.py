...
# 加载模型
MODEL_PATH = "save/butterfly_model_85%.pkl"  # 确保路径正确
try:
...
if model is not None:
    interface = gr.Interface(
        fn=predict_image,
        inputs=gr.Image(type="numpy", label="上传蝴蝶图片", height=300),
        outputs=[
            gr.Textbox(label="预测结果", lines=2, max_lines=2),
            gr.Image(label="置信度分布图", height=400)
        ],
        title="蝴蝶分类系统",
        description="上传一张蝴蝶图片，系统将预测其类别并显示置信度分布。"
    )
